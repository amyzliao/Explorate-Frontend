...
export default {
    name: 'DatabaseCard',
    props: ['name', 'location', 'mission', 'projects', 'link', 'id']
}
</script>
...
<template>
    <div class="bg-white border border-gray-200 rounded-xl shadow">
        <!-- Title section -->
        <router-link :to="'/database/' + id">
            <div class="h-[72px] flex items-center gap-4 py-3 pl-4 pr-1">
                <!-- profile pic circle, just placeholder with first initial -->
                <div
                    class="h-10 w-10 bg-blue-400 rounded-full flex justify-center items-center text-white font-bold text-2xl">
                    {{ name[0] }}
                </div>
                <!-- NGO name -->
        ...
        <div class="p-2">
            ...
            <div class="flex justify-end gap-2 m-4">
                <div class="border rounded-[100px] py-2.5 px-6 font-medium text-[#5D90CD] border-[#79747E] text-sm">Save
                </div>
